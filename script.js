let producoesDoDia = [];

function fazerLogin() {
    const user = document.getElementById('login-email').value;
    if(user) {
        document.getElementById('user-display').innerText = user.toUpperCase();
        document.getElementById('tela-login').style.display = 'none';
        document.getElementById('app-principal').style.display = 'block';
    } else {
        alert("Digite um ID");
    }
}

function abrirModulo(nome) {
    document.getElementById('grid-home').style.display = 'none';
    document.getElementById('conteudo-modulo').style.display = 'block';
    const titulos = { injecao: "INJEÇÃO", bobines: "BOBINES", serra: "SERRA", embalagem: "EMBALAGEM", gestao: "GESTÃO", config: "AJUSTES" };
    document.getElementById('titulo-modulo').innerText = titulos[nome];

    document.getElementById('render-modulo').innerHTML = `
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; padding:15px;">
            <div class="card" onclick="exibirFormulario('${nome}')"><i class="fas fa-plus"></i><span>Novo Relatório</span></div>
            <div class="card" onclick="exibirHistoricoModulo('${nome}')"><i class="fas fa-history"></i><span>Histórico</span></div>
        </div>`;
}

function voltarHome() {
    document.getElementById('conteudo-modulo').style.display = 'none';
    document.getElementById('grid-home').style.display = 'grid';
    producoesDoDia = [];
}


    producoesDoDia.push(item);
    document.getElementById('btn-finalizar').style.display = "block";
    document.getElementById('lista-temp').innerHTML += `<div style="padding:10px; border-bottom:1px solid #334155; font-size:13px;">• ${item.nome} - ${item.metros}m</div>`;
    
    // Limpa campos rápidos
    document.getElementById('prod-nome').value = "";
    document.getElementById('prod-metros').value = "";
    let tabelaItens = "";

producoesDoDia.forEach(item => {
    // Linha principal do produto
    tabelaItens += `
        <tr style="background:#fff;">
            <td style="border:1px solid #000; padding:5px;">${item.chapa || item.tipo || 'N/A'}</td>
            <td style="border:1px solid #000; padding:5px;">${item.espessura || '-'}</td>
            <td style="border:1px solid #000; padding:5px;">${item.metros}m</td>
            <td style="border:1px solid #000; padding:5px;">${item.vel}</td>
            <td style="border:1px solid #000; padding:5px;">${item.pol}</td>
            <td style="border:1px solid #000; padding:5px;">${item.mdi}</td>
            <td style="border:1px solid #000; padding:5px;">${item.pen}</td>
            <td style="border:1px solid #000; padding:5px;">${item.cat1 || 0}</td>
            <td style="border:1px solid #000; padding:5px;">${item.cat2 || 0}</td>
            <td style="border:1px solid #000; padding:5px;">${item.cat3 || 0}</td>
            <td style="border:1px solid #000; padding:5px;">${item.cat4 || 0}</td>
        </tr>
    `;

    // Se houver paragens, adiciona uma linha especial logo abaixo do produto
    if (item.paragens && item.paragens.length > 0) {
        let paragensTexto = item.paragens.map(p => `<b>${p.motivo}</b> (${p.tempo} min)`).join(" | ");
        
        tabelaItens += `
            <tr>
                <td colspan="11" style="border:1px solid #000; padding:4px; background:#f9fafb; font-size:9px; color:#334155;">
                    <i class="fas fa-stop-circle"></i> <b>PARAGENS:</b> ${paragensTexto}
                </td>
            </tr>
        `;
    }
});
function exibirFormulario(modulo) {
    const hoje = new Date().toISOString().split('T')[0];
    document.getElementById('render-modulo').innerHTML = `
        <div style="padding: 15px;">
            <label style="font-size:12px; color:#94a3b8;">DATA DA PRODUÇÃO</label>
            <input type="date" id="data-producao" value="${hoje}" style="width:100%; padding:12px; background:#020617; color:white; border:1px solid #334155; border-radius:8px; margin-bottom:15px;">
            
            <label style="font-size:12px; color:#94a3b8;">TIPO DE PAINEL</label>
            <select id="inj-painel">
                <option value="5 Ondas">5 Ondas</option>
                <option value="Telha Canudo">Telha Canudo</option>
                <option value="Fachada oculta">Fachada Oculta</option>
                <option value="Fachada visivel">Fachada visivel</option>
                <option value="Fachada ondulada">Fachada ondulada</option>
                <option value="Polipainel">Polipainel</option>
            </select>
            
            <div style="display:flex; gap:10px; margin-bottom:10px;">
                <div style="flex:1;">
                    <label style="font-size:12px; color:#94a3b8;">ESPESSURA</label>
                    <select id="inj-esp">
                        <option value="20">20 mm</option>
                        <option value="30">30 mm</option>
                        <option value="40">40 mm</option>
                        <option value="50">50 mm</option>
                        <option value="60">60 mm</option>
                        <option value="80">80 mm</option>
                        <option value="100">100 mm</option>
                        <option value="120">120 mm</option>
                    </select>
                </div>
                <div style="flex:1;">
                    <label style="font-size:12px; color:#94a3b8;">METROS</label>
                    <input type="number" id="prod-metros" placeholder="Qtd" style="width:100%; padding:12px; background:#020617; color:white; border:1px solid #334155; border-radius:8px;">
                </div>
            </div>

            <label style="font-size:11px; color:#3b82f6; font-weight:bold;">PARÂMETROS DA MÁQUINA</label>
            <div class="grid-quimicos">
                <input type="number" id="q-pol" placeholder="POL kg">
                <input type="number" id="q-mdi" placeholder="MDI kg">
                <input type="number" id="q-pen" placeholder="PEN kg">
                <input type="number" id="q-cat 1" placeholder="Cat 1 kg">
               <input type="number" id="q-cat 2" placeholder="Cat 2 kg">
               <input type="number" id="q-cat 3" placeholder="Cat 3 kg">
               <input type="number" id="q-cat 4" placeholder="Cat 4 kg">
                
                
                <select id="q-vel" style="margin-bottom: 0;">
                    <option value="" disabled selected>Vel (m/min)</option>
                    <option value="5">5 m/min</option>
                    <option value="6">6 m/min</option>
                    <option value="8">8 m/min</option>
                    <option value="9">9 m/min</option>
                    <option value="10">10 m/min</option>
                    <option value="11">11 m/min</option>
                    <option value="12">12 m/min</option>
                </select>
            </div>

            <button onclick="salvarNaLista()" class="btn-primary btn-add-lista">ADICIONAR À LISTA</button>
            <div id="lista-temp" style="margin-top:15px;"></div>
            <button id="btn-finalizar" onclick="finalizarTurno('${modulo}')" class="btn-primary btn-finish-dia" style="display:none;">SALVAR RELATÓRIO DO DIA</button>
        </div>`;
}

function salvarNaLista() {
    const painel = document.getElementById('inj-painel').value;
    const esp = document.getElementById('inj-esp').value;
    const metros = document.getElementById('prod-metros').value;

    if(!metros) return alert("Por favor, insira a quantidade de metros!");

    const idUnico = Date.now();

    const item = {
        id: idUnico,
        nome: painel,
        esp: esp,
        metros: metros,
        pol: document.getElementById('q-pol').value || 0,
        mdi: document.getElementById('q-mdi').value || 0,
        pen: document.getElementById('q-pen').value || 0,
        c1: document.getElementById('q-cat 1').value || 0,
        c2: document.getElementById('q-cat 2').value || 0,
        c3: document.getElementById('q-cat 3').value || 0,
        c4: document.getElementById('q-cat 4').value || 0,
        vel: document.getElementById('q-vel').value || 0
    };

    producoesDoDia.push(item);
    document.getElementById('btn-finalizar').style.display = "block";
    
    atualizarListaVisual(); // Chamamos uma função para desenhar a lista
    
    // Limpa apenas o campo de metros
    document.getElementById('prod-metros').value = "";
}
function atualizarListaVisual() {
    const listaTemp = document.getElementById('lista-temp');
    listaTemp.innerHTML = ""; // Limpa para redesenhar atualizado

    producoesDoDia.forEach(item => {
        listaTemp.innerHTML += `
            <div style="padding:10px; border-bottom:1px solid #334155; background:#1e293b; margin-bottom:5px; border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
                <div style="font-size:12px; color:white;">
                    <b>${item.nome} (${item.esp}mm)</b><br>
                    <span>Mts: ${item.metros} | Vel: ${item.vel} | POL: ${item.pol} | MDI: ${item.mdi} | PEN: ${item.pen}</span>
                </div>
                <div style="display:flex; gap:5px;">
                    <button onclick="editarTudo(${item.id})" style="background:#eab308; color:black; border:none; padding:5px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:10px;">EDITAR</button>
                    <button onclick="removerItem(${item.id})" style="background:#ef4444; color:white; border:none; padding:5px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:10px;">X</button>
                </div>
            </div>`;
    });
}

function editarTudo(id) {
    // 1. Acha o item no array de produções
    const item = producoesDoDia.find(p => p.id === id);
    if(!item) return;

    // 2. Seleciona onde os inputs vão aparecer dentro do modal
    const container = document.getElementById('conteudo-modal');

    // 3. Limpa e insere todos os campos para edição simultânea
container.innerHTML = `
    <input type="hidden" id="edit-id" value="${item.id}">
    
    <div style="display:flex; flex-direction:column; gap:12px; padding: 5px; max-height: 75vh; overflow-y: auto; overflow-x: hidden;">
        
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; background: #1e293b; padding: 10px; border-radius: 8px; border-left: 4px solid #10b981;">
            <div>
                <label style="font-size:10px; color:#94a3b8; display:block; font-weight:bold;">ESPESSURA</label>
                <select id="inj-esp">
                        <option value="20">20 mm</option>
                        <option value="30">30 mm</option>
                        <option value="40">40 mm</option>
                        <option value="50">50 mm</option>
                        <option value="60">60 mm</option>
                        <option value="80">80 mm</option>
                        <option value="100">100 mm</option>
                        <option value="120">120 mm</option>
                    </select>
            </div>
            <div>
                <label style="font-size:10px; color:#94a3b8; display:block; font-weight:bold;">TIPO DE CHAPA</label>
                <select id="inj-painel">
                <option value="5 Ondas">5 Ondas</option>
                <option value="Telha Canudo">Telha Canudo</option>
                <option value="Fachada oculta">Fachada Oculta</option>
                <option value="Fachada visivel">Fachada visivel</option>
                <option value="Fachada ondulada">Fachada ondulada</option>
                <option value="Polipainel">Polipainel</option>
            </select>
            </div>
        </div>

        <div style="border: 1px solid #334155; border-radius: 8px; padding: 10px; background: #0f172a;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 10px;">
                <label style="font-size:11px; color:#94a3b8; font-weight:bold;">PARAGENS NESTE MATERIAL</label>
                <button type="button" onclick="adicionarLinhaParagem()" style="background:#10b981; color:white; border:none; border-radius:4px; padding:3px 8px; cursor:pointer; font-size:10px; font-weight:bold;">+ ADICIONAR</button>
            </div>
            <div id="container-paragens"></div>
        </div>

        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
             <div style="display:flex; flex-direction:column; gap:3px;">
                <label style="color:#94a3b8; font-size:10px; font-weight:bold;">METROS</label>
                <input type="number" id="edit-metros" value="${item.metros}" style="width:100%; padding:8px; border-radius:6px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div style="display:flex; flex-direction:column; gap:3px;">
                <label style="color:#94a3b8; font-size:10px; font-weight:bold;">VELOCIDADE</label>
                <select id="q-vel" style="margin-bottom: 0;">
                    <option value="" disabled selected>Vel (m/min)</option>
                    <option value="5">5 m/min</option>
                    <option value="6">6 m/min</option>
                    <option value="8">8 m/min</option>
                    <option value="9">9 m/min</option>
                    <option value="10">10 m/min</option>
                    <option value="11">11 m/min</option>
                    <option value="12">12 m/min</option>
                </select>
            </div>
            </div>
        </div>

        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:8px;">
            <div>
                <label style="color:#94a3b8; font-size:10px;">POL</label>
                <input type="number" id="edit-pol" value="${item.pol || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">MDI</label>
                <input type="number" id="edit-mdi" value="${item.mdi || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">PEN</label>
                <input type="number" id="edit-pen" value="${item.pen || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">Cat 1</label>
                <input type="number" id="edit-cat1" value="${item.cat1 || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">Cat 2</label>
                <input type="number" id="edit-cat2" value="${item.cat2 || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">Cat 3</label>
                <input type="number" id="edit-cat3" value="${item.cat3 || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
            <div>
                <label style="color:#94a3b8; font-size:10px;">Cat 4</label>
                <input type="number" id="edit-cat4" value="${item.cat4 || 0}" style="width:100%; padding:6px; border-radius:5px; border:1px solid #334155; background:#020617; color:white;">
            </div>
        </div>
`;
        
`;

// Chamar a função para carregar as paragens LOGO APÓS preencher o innerHTML
if (item.paragens && item.paragens.length > 0) {
    item.paragens.forEach(p => adicionarLinhaParagem(p.motivo, p.tempo));
} else {
    adicionarLinhaParagem(); 
}

    // 4. Mostra o modal (muda de none para flex)
    document.getElementById('modal-edicao').style.display = 'flex';
}

function removerItem(id) {
    if(confirm("Excluir este lançamento?")) {
        producoesDoDia = producoesDoDia.filter(p => p.id !== id);
        atualizarListaVisual();
        if(producoesDoDia.length === 0) document.getElementById('btn-finalizar').style.display = "none";
    }
}
function editarItemLista(id) {
    // Busca o item no array producoesDoDia
    const item = producoesDoDia.find(p => p.id === id);
    
    const novoMetros = prompt("Corrigir metros:", item.metros);
    
    if (novoMetros !== null && novoMetros !== "") {
        // Atualiza no array (para o PDF sair certo)
        item.metros = novoMetros;
        // Atualiza na tela para você ver a mudança
        document.getElementById(`txt-metros-${id}`).innerText = novoMetros + "m";
    }
}

function removerItemLista(id) {
    if (confirm("Deseja remover este lançamento?")) {
        // Remove do array
        producoesDoDia = producoesDoDia.filter(p => p.id !== id);
        // Remove da tela
        const elemento = document.getElementById(`bloco-${id}`);
        elemento.remove();
        
        // Esconde o botão de finalizar se a lista ficar vazia
        if (producoesDoDia.length === 0) {
            document.getElementById('btn-finalizar').style.display = "none";
        }
    }
}
// Dentro da sua função que adiciona o item na lista da tela:
function adicionarItemNaTela(item) {
    const lista = document.getElementById('lista-producao');
    
    const div = document.createElement('div');
    div.className = "item-producao";
    div.style = "margin-bottom: 10px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 5px;";

    div.innerHTML = `
        <span><b>${item.nome}</b></span>
        
        <label>Metros: <input type="number" value="${item.metros}" class="edit-metros" style="width: 50px;"></label>
        <label>Vel: <input type="number" value="${item.velocidade}" class="edit-vel" style="width: 50px;"></label>
        
        <div style="grid-column: span 3; font-size: 0.8em; color: #ccc;">
            POL: <input type="number" value="${item.pol}" class="edit-pol" style="width: 40px;"> kg | 
            MDI: <input type="number" value="${item.mdi}" class="edit-mdi" style="width: 40px;"> kg | 
            PEN: <input type="number" value="${item.pen}" class="edit-pen" style="width: 40px;"> kg
        </div>
        <hr style="grid-column: span 3; width: 100%; border: 0.5px solid #444;">
    `;
    
    lista.appendChild(div);
}

function finalizarTurno(modulo) {
    const data = document.getElementById('data-producao').value;
    const d = new Date(data + "T12:00:00");
    const ano = d.getFullYear();
    const mes = d.toLocaleString('pt-br', { month: 'long' }).toUpperCase();
    
    let db = JSON.parse(localStorage.getItem('atlas_db')) || {};
    if(!db[ano]) db[ano] = {};
    if(!db[ano][mes]) db[ano][mes] = [];

    db[ano][mes].push({
        modulo,
        data: d.toLocaleDateString('pt-br'),
        operador: document.getElementById('user-display').innerText,
        itens: [...producoesDoDia]
    });

    localStorage.setItem('atlas_db', JSON.stringify(db));
    alert("Relatório salvo com sucesso!");
    voltarHome();
}

function exibirHistoricoModulo(modulo) {
    const db = JSON.parse(localStorage.getItem('atlas_db')) || {};
    const render = document.getElementById('render-modulo');
    let html = "";

    for(let ano in db) {
        html += `<div class="folder-year" onclick="this.nextElementSibling.style.display = this.nextElementSibling.style.display === 'none' ? 'block' : 'none'">📂 ANO ${ano}</div><div style="display:none; padding:10px;">`;
        for(let mes in db[ano]) {
            const filtrados = db[ano][mes].filter(r => r.modulo === modulo);
            if(filtrados.length > 0) {
                html += `<div class="folder-month" onclick="this.nextElementSibling.style.display = this.nextElementSibling.style.display === 'none' ? 'block' : 'none'">📅 ${mes}</div><div style="display:none;">`;
                filtrados.forEach((rel, idx) => {
                    const idRel = `rel-${modulo}-${idx}`;
                    html += `
                        <div class="history-day-card">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <b>${rel.data}</b>
                                <div style="display:flex; gap:5px;">
                                    <button onclick="document.getElementById('${idRel}').style.display = (document.getElementById('${idRel}').style.display === 'none' ? 'block' : 'none')" style="background:#475569; color:white; border:none; padding:8px; border-radius:5px; font-size:10px;">VER</button>
                                    <button onclick="gerarPDF('${idRel}', '${rel.data}')" style="background:#10b981; color:white; border:none; padding:8px; border-radius:5px; font-size:10px;">PDF</button>
                                </div>
                            </div>
                            <div id="${idRel}" style="display:none; margin-top:10px; padding:10px; background:#1e293b; border-radius:5px; font-size:12px;">
                                <p style="color:#94a3b8; margin-bottom:8px;">Operador: ${rel.operador}</p>
                                ${rel.itens.map(i => `
                                    <div style="margin-bottom:8px; border-bottom:1px solid #334155; padding-bottom:5px;">
                                        <strong>${i.nome}</strong> (${i.esp}mm)<br>
                                        Qtd: ${i.metros}m | Vel: ${i.vel}m/min<br>
                                        <small>Químicos: P:${i.pol} M:${i.mdi} Pen:${i.pen}</small>
                                    </div>
                                `).join('')}
                            </div>
                        </div>`;
                });
                html += `</div>`;
            }
        }
        html += `</div>`;
    }
    render.innerHTML = html || "<p style='text-align:center; padding:20px; color:gray;'>Nenhum histórico.</p>";
}



    // Agora o PDF vai sair porque a imagem já faz parte do texto!
    html2pdf().set(opcoes).from(conteudoPDF).save();
function gerarPDF(id, dataRelatorio) {
    const elementoOriginal = document.getElementById(id);
    const nomeUsuario = document.getElementById('user-display').innerText || "Operador";

    let tabelaItens = "";
    const blocos = elementoOriginal.querySelectorAll('div[style*="margin-bottom:8px"]');

    blocos.forEach(bloco => {
        const texto = bloco.innerText;
        
        // Extração dos dados
        const nomeP = texto.split('(')[0].trim();
        const esp = texto.includes('(') ? texto.split('(')[1].split('mm')[0] + 'mm' : '-';
        const metros = texto.includes('Qtd:') ? texto.split('Qtd:')[1].split('m')[0].trim() : '';
        const vel = texto.includes('Vel:') ? texto.split('Vel:')[1].split('m/min')[0].trim() : '';
        const pol = texto.includes('P:') ? texto.split('P:')[1].split(' ')[0] : '';
        const mdi = texto.includes('M:') ? texto.split('M:')[1].split(' ')[0] : '';
        const pen = texto.includes('Pen:') ? texto.split('Pen:')[1].split(' ')[0].trim() : '';

        // Captura Catalisadores
        const cat1 = texto.includes('C1:') ? texto.split('C1:')[1].split(' ')[0] : '';
        const cat2 = texto.includes('C2:') ? texto.split('C2:')[1].split(' ')[0] : '';
        const cat3 = texto.includes('C3:') ? texto.split('C3:')[1].split(' ')[0] : '';
        const cat4 = texto.includes('C4:') ? texto.split('C4:')[1].trim() : '';
     const paragensDiv = bloco.querySelector('.paragens-historico');
        const textoParagens = paragensDiv ? paragensDiv.innerText.replace('⚠️ Paragens:', '').trim() : '';
        


        tabelaItens += `
            <tr>
                <td style="border: 1px solid #000; padding: 5px;">${nomeP}</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${esp}</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${metros} metros</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${vel} m/min</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${pol} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${mdi} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${pen} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${cat1} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${cat2} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${cat3} kg</td>
                <td style="border: 1px solid #000; padding: 5px; text-align:center;">${cat4} kg</td>
            </tr>`;
    });

    const conteudoFinal = document.createElement('div');
    conteudoFinal.style.padding = "15px";
    conteudoFinal.style.color = "black";
    conteudoFinal.style.background = "white";

    conteudoFinal.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #000; padding-bottom:10px;">
            <img src="data:image/png;base64," style="width: 120px; height: auto;">
            
            <div style="text-align:right; font-family: sans-serif;">
                <h3 style="margin:0;">RELATÓRIO DA INJEÇÃO</h3>
                <p style="margin:0;"><b>Data:</b> ${dataRelatorio} | <b>Finalizado por:</b> ${nomeUsuario}</p>
            </div>
        </div>

        <table style="width:100%; border-collapse:collapse; margin-top:20px; font-family:sans-serif; font-size:10px;">
            <thead>
                <tr style="background:#f2f2f2;">
                    <th style="border:1px solid #000; padding:5px;">PRODUTO</th>
                    <th style="border:1px solid #000; padding:5px;">ESP.</th>
                    <th style="border:1px solid #000; padding:5px;">METROS</th>
                    <th style="border:1px solid #000; padding:5px;">VELOCIDADE</th>
                    <th style="border:1px solid #000; padding:5px;">POL</th>
                    <th style="border:1px solid #000; padding:5px;">MDI</th>
                    <th style="border:1px solid #000; padding:5px;">PEN</th>
                    <th style="border:1px solid #000; padding:5px;">CAT 1</th>
                    <th style="border:1px solid #000; padding:5px;">CAT 2</th>
                    <th style="border:1px solid #000; padding:5px;">CAT 3</th>
                    <th style="border:1px solid #000; padding:5px;">CAT 4</th>
                </tr>
            </thead>
            <tbody>
                ${tabelaItens}
            </tbody>
        </table>
    `;

    const opt = {
        margin: 5,
        filename: 'Relatorio_Injecao.pdf',
        html2canvas: { scale: 2, useCORS: true }, // CORS ajuda a carregar a imagem interna
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }
    };

    html2pdf().set(opt).from(conteudoFinal).toPdf().get('pdf').then(function (pdf) {
        window.open(pdf.output('bloburl'), '_blank');
    });
}
function adicionarProducao() {
    const lista = document.getElementById('lista-producao'); // Confira se o ID da sua div é este
    
    // Captura os valores digitados nos campos azuis
    const produto = document.querySelector('input[placeholder="Tipo de Painel"]').value;
    const espessura = document.querySelector('input[placeholder="Espessura"]').value;
    const metros = document.querySelector('input[placeholder="Metros"]').value;

    const divItem = document.createElement('div');
    divItem.className = "item-lancado";
    divItem.style = "background: #1e293b; padding: 10px; border-radius: 8px; margin-bottom: 5px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #3b82f6;";

    // Criamos a linha com os valores e os botões de EDITAR e EXCLUIR
    divItem.innerHTML = `
        <div class="dados-item" style="color: white; font-weight: bold;">
            <span class="txt-nome">${produto} (${espessura})</span> - 
            <span class="txt-metros">${metros}m</span>
        </div>
        
        <div class="acoes">
            <button onclick="editarLinha(this)" style="background: #eab308; border: none; color: black; border-radius: 4px; padding: 4px 8px; cursor: pointer; margin-right: 5px; font-weight: bold;">EDITAR</button>
            <button onclick="this.parentElement.parentElement.remove()" style="background: #ef4444; border: none; color: white; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-weight: bold;">X</button>
        </div>
    `;

    lista.appendChild(divItem);
}

// Função para transformar o texto em campos editáveis na hora
function editarLinha(botao) {
    const divItem = botao.parentElement.parentElement;
    const dadosDiv = divItem.querySelector('.dados-item');
    
    // Pega o texto atual
    const textoAtual = dadosDiv.innerText;
    
    // Transforma o texto em um campo de entrada (input)
    dadosDiv.innerHTML = `<input type="text" value="${textoAtual}" style="background: #0f172a; color: #10b981; border: 1px solid #3b82f6; width: 250px; padding: 5px; font-weight: bold;">`;
    
    // Muda o botão de "EDITAR" para "SALVAR"
    botao.innerText = "SALVAR";
    botao.style.background = "#10b981";
    botao.style.color = "white";
    botao.onclick = function() { salvarLinha(this); };
}

function salvarLinha(botao) {
    const divItem = botao.parentElement.parentElement;
    const novoValor = divItem.querySelector('input').value;
    
    divItem.querySelector('.dados-item').innerHTML = novoValor;
    
    // Volta o botão para o estado original de EDITAR
    botao.innerText = "EDITAR";
    botao.style.background = "#eab308";
    botao.style.color = "black";
    botao.onclick = function() { editarLinha(this); };
}
function salvarEdicaoModal() {
    try {
        const idInput = document.getElementById('edit-id');
        if (!idInput) return;

        const id = parseInt(idInput.value);
        // Procure o item no seu array - Verifique se o nome é 'producoesDoDia'
        const item = producoesDoDia.find(p => p.id === id);

        if (item) {
            // Função auxiliar para pegar valor com segurança
            const getVal = (id) => document.getElementById(id) ? document.getElementById(id).value : "";

            // Atualizando os dados básicos
            item.espessura = getVal('edit-espessura');
            item.chapa = getVal('edit-chapa');
            item.metros = getVal('edit-metros');
            item.vel = getVal('edit-vel');
            
            // Atualizando Químicos (POL, MDI, PEN)
            item.pol = getVal('edit-pol');
            item.mdi = getVal('edit-mdi');
            item.pen = getVal('edit-pen');

            // Atualizando CATs
            item.cat1 = getVal('edit-cat1');
            item.cat2 = getVal('edit-cat2');
            item.cat3 = getVal('edit-cat3');
            item.cat4 = getVal('edit-cat4');

            // Coletando Paragens
            const motivos = document.querySelectorAll('.paragem-motivo');
            const tempos = document.querySelectorAll('.paragem-tempo');
            item.paragens = [];
            
            motivos.forEach((el, index) => {
                if (el.value.trim() !== "") {
                    item.paragens.push({ 
                        motivo: el.value, 
                        tempo: tempos[index] ? tempos[index].value : "0" 
                    });
                }
            });

            fecharModal();
            
            // Atualiza a lista na tela (se a sua função tiver outro nome, mude aqui)
            if (typeof atualizarListaVisual === "function") {
                atualizarListaVisual();
            }

            console.log("Salvo com sucesso!", item);
        }
    } catch (erro) {
        console.error("Erro detalhado:", erro);
        alert("Erro técnico: " + erro.message);
    }
}
function fecharModal() {
    const modal = document.getElementById('modal-edicao');
    if (modal) {
        modal.style.display = 'none';
    }
}
function adicionarLinhaParagem(motivo = '', tempo = '') {
    const container = document.getElementById('container-paragens');
    if (!container) return; 

    const div = document.createElement('div');
    div.style = "display:grid; grid-template-columns: 1fr 80px 30px; gap:8px; align-items:center; margin-bottom: 5px;";
    
    div.innerHTML = `
        <input type="text" placeholder="Motivo da parada" value="${motivo}" class="paragem-motivo" style="padding:6px; border-radius:4px; border:1px solid #334155; background:#0f172a; color:white; font-size:12px;">
        <input type="number" placeholder="Min" value="${tempo}" class="paragem-tempo" style="padding:6px; border-radius:4px; border:1px solid #334155; background:#0f172a; color:white; font-size:12px;">
        <button type="button" onclick="this.parentElement.remove()" style="background:#ef4444; color:white; border:none; border-radius:4px; cursor:pointer; height:28px; font-weight:bold;">X</button>
    `;
    
    container.appendChild(div);
}
